...
public class TelegramBot extends TelegramLongPollingBot {
    ...
    private String setBase(String userId, String cryptoId, float base){
        ...
        if (!SupportedCrypto.getSupportedCrypto().isSupportedCrypto(cryptoId))  return "Cannot define crypto code";
        try{
            user_lock.acquire();
            for (User user : userList) {
                if (user.getId().equals(userId)) {
                    try{
                        ...
                        user.updateWatchedCrypto(new WatchedCrypto(cryptoId, base)); //lock here, modify user's crypto list
            ...
            try{
                ...
                for (User user : userList) {///iterating, lock
                    if (user.getId().equals(userId)) {
                        try{
                            crypto_list_lock.acquire();
                            boolean success = user.deleteWatchedCrypto(cryptoId); //lock here, modify user's crypto list
                            if (success) return "Successfully stop watching " + cryptoId;
                            else return "You have not watched " + cryptoId + " yet";
                        }catch(Exception e){
                            e.printStackTrace();
                            return "Cannot update due to some error";
                        }finally{
                    ...
                    if(user.getWatchedCryptoList().size() > 0){
                        try{
                            ...
                            for (WatchedCrypto watchedCrypto : user_list){////iterating, lock
                                ...
                                if(watchedCrypto.isLowerThanBase(crypto.getCurrentPrice()) && !watchedCrypto.isWarned()){
